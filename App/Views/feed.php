...
$listData['_feed_link'] = Info::getMttinfo('mtt_url') . "feed?list=" . (int)$listData['id'] . ($feedType != '' ? "&feed=" . $feedType : '');
$listData['_feed_type'] = $feedType;

Html::htmlarrayRef($listData);

printRss($data, $listData);

/* ===================================================================================================================== */

function fillData(array &$data, int $listId, string $field, string $sqlWhere): void
{
    ...
    foreach ($tasks as $r) {
        ...
        if ($r['duedate'] != '') {
            ...
            $a[] = $lang->get('due') . ": " . Formatter::formatDate3(Config::get('dateformat'), (int)$ad[0], (int)$ad[1], (int)$ad[2], $lang);
        }
        ...
        if ($r['compl']) {
            $a[] = $lang->get('taskdate_completed') . ": " . Formatter::timestampToDatetime($r['d_completed']);
        }

        $r['title'] = htmlspecialchars($r['title']);
        $r['note'] = MTTMarkdown::noteMarkup($r['note'], true);
        $r['_descr'] = implode("<br/>", Html::htmlarray($a)) . "<br/><br/>" . $r['note'];
...
function printRss(array $data, array $listData): void
{
    $lang = Lang::instance();
    $link = Info::getMttinfo('url') . "?list=" . (int)$listData['id'];
    $buildDate = gmdate('r');
