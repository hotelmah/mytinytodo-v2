...
if ($listData && !Authentication::isLogged() && !$listData['published']) {
...
function printCSV(array $listData, array $data): void
{
...
function escape_csv(string $v): string
{
    //escape formulas
    $nf = '';
    $trimmed = ltrim($v);
...
function printICal(array $listData, array $data): void
{
...
/* ===================================================================================================================== */
/**
 * Splits a UTF-8 string into chunks of specified length.
 *
 * @param string $text The input text to be split.
 * @param int $chunklen The maximum length of each chunk in bytes.
 * @param string $delimiter The delimiter to use between chunks.
 * @return string The text split into chunks, joined by the specified delimiter.
 */
