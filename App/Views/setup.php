...
use App\Database\DatabaseAbstract;
...
define('DB_CONFIG_FILE', 'App' . DIRECTORY_SEPARATOR . 'Config' . DIRECTORY_SEPARATOR . 'config.database.php');
...
$mttVersion = htmlspecialchars(Version::VERSION);
...
if ($configExists) {
    ...
    if (!$db) {
        exitMessage("Database connection config file seems to be incorrect. You can remove config.php or edit it manually and then reload setup.<br><br>" . "<b>Error:</b> " . htmlspecialchars($error));
    }
    ...
    echo "Done. Setup Complete.<br><br><b>Attention!</b> Delete this file for security reasons. <br><br>Go to <a href='" . htmlspecialchars(UrlPath::urlDir(UrlPath::getRequestUri())) . "'>homepage</a>.<br><br>";
    printFooter();
}

if ($ver == '') {
    $install = trim(Request::post('install'));
    ...
    if ($install == '' && $db !== null) {
        ...
        if ($dbtype == 'mysql' || $dbtype == 'postgres') {
            Config::set('db.host', Request::post('db_host'));
            Config::set('db.name', Request::post('db_name'));
            Config::set('db.user', Request::post('db_user'));
            Config::set('db.password', Request::post('db_password'));
            Config::set('db.prefix', trim(Request::post('db_prefix')));
        }
        ...
        # create default list
        $db->ex("INSERT INTO {$db->prefix}lists (uuid,name,d_created,taskview) VALUES (?,?,?,?)", array(Security::generateUUID(), 'Todo', time(), 1));
        DBConnection::init($db);
        Config::save();
    } else {
...
function update_stoken()
{
    ...
    if (PHP_VERSION_ID < 70300) {
        setcookie('mtt-s-token', $token, 0, UrlPath::urlDir(UrlPath::getRequestUri()) . '; samesite=lax', '', false, true);
    } else {
        setcookie('mtt-s-token', $token, [
            'path' => UrlPath::urlDir(UrlPath::getRequestUri()),
            'httponly' => true,
            'samesite' => 'lax'
        ]);
    }
...
function createMysqlTables(DatabaseAbstract $db)
{
    ...
    $db->ex("CREATE TABLE {$db->prefix}settings (
    ...
    $db->ex("CREATE TABLE {$db->prefix}sessions (
...
function createPostgresTables(DatabaseAbstract $db)
{
    $db->ex("CREATE TABLE {$db->prefix}lists (
...
function createSqliteTables(DatabaseAbstract $db)
{
    $db->ex("CREATE TABLE {$db->prefix}lists (
...
function databaseVersion(DatabaseAbstract $db): string
{
    if (!$db) {
        return '';
    }

    if (!$db->tableExists($db->prefix . 'todolist')) {
        return '';
    }

    $v = '1.0';
    if (!$db->tableExists($db->prefix . 'tags')) {
        return $v;
    }

    $v = '1.1';
    if (!$db->tableFieldExists($db->prefix . 'todolist', 'duedate')) {
        return $v;
    }

    $v = '1.2';
    if (!$db->tableExists($db->prefix . 'lists')) {
        return $v;
    }

    $v = '1.3.0';
    if (!$db->tableFieldExists($db->prefix . 'todolist', 'd_completed')) {
        return $v;
    }

    $v = '1.3.1';
    if (!$db->tableFieldExists($db->prefix . 'todolist', 'd_edited')) {
        return $v;
    }

    $v = '1.4';
    if (!$db->tableExists($db->prefix . 'settings')) {
        return $v;
    }

    $v = '1.7';
    if ($db->tableFieldExists($db->prefix . 'todolist', 'tags')) {
        return $v;
    }
...
function hasMysqlUnicode520(DatabaseAbstract $db): bool
{
...
function testConnect(&$error)
{
    ...
    try {
        ...
        if (MTT_DB_TYPE == 'mysql') {
            ...
            if (false === $f = @fopen('todolist.db', 'a+')) {
            ...
            $db = new DatabaseSqlite3(['filename' => 'todolist.db']);
...
function databaseTypeName(DatabaseAbstract $db)
{
    switch ($db::DBTYPE) {
...
function update_14_17(DatabaseAbstract $db, $dbtype)
{
    $db->ex("BEGIN");
...
function update_17_18(DatabaseAbstract $db, $dbtype)
{
    $db->ex("BEGIN");
