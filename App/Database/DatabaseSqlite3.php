...
class DatabaseSqlite3 extends DatabaseAbstract
{
    const DBTYPE = 'sqlite';
    ...
    public function connect(array $params): void
    {
        ...
        $this->dbh->sqliteCreateFunction('utf8Lower', [$this, 'utf8Lower'], 1);
        $this->dbh->sqliteCreateFunction('utf8NormalizedLower', [$this, 'utf8NormalizedLower'], 1);
        $this->dbh->sqliteCreateCollation('UTF8CI', [$this, 'collateUtf8ci']);
        $this->dbh->sqliteCreateCollation('UTF8CI_NORMALIZED', [$this, 'collateUtf8ciNormalized']);
    }
    ...
    */
    ...
    private function dQuery(string $query, ?array $values = null, bool $resultless = false): DatabaseResultAbstract
    {
        ...
        $dbr = new DatabaseResultSqlite3($this->dbh, $query, $resultless);
    ...
    public function like(string $column, string $format, string $string): string
    {
        ...
        if ($this->useNormalizedUtf8) {
            return 'utf8NormalizedLower("' . $column . '") LIKE ' . $this->quoteForLike($format, $this->utf8NormalizedLower($string));
        }
        return 'utf8Lower("' . $column . '") LIKE ' . $this->quoteForLike($format, $this->utf8Lower($string));
    }
    ...
    public function ciEquals(string $column, string $value): string
    {
        $column = str_replace('"', '""', $column);
        if ($this->useNormalizedUtf8) {
            return 'utf8NormalizedLower("' . $column . '") = ' . $this->quote($this->utf8NormalizedLower($value));
        }
        return 'utf8Lower("' . $column . '") = ' . $this->quote($this->utf8Lower($value));
    }
    ...
    public function utf8Lower($value): string
    {
        if (is_null($value)) {
            return '';
        }
    ...
    public function utf8NormalizedLower($value): string
    {
        if (is_null($value)) {
            return '';
        }
    ...
    public function collateUtf8ci(string $str1, string $str2): int
    {
        return strcmp(mb_strtolower($str1, 'UTF-8'), mb_strtolower($str2, 'UTF-8'));
    }

    public function collateUtf8ciNormalized(string $str1, string $str2): int
    {
