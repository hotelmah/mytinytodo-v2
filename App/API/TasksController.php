    ...
    public function get(array $args = [])
    {
        ...
        if ($tag != '') {
            ...
            if (count($tagIds) > 0) {
                ...
                $sqlWhere .= "\n AND todo.id IN (" . "SELECT DISTINCT task_id FROM {$db->prefix}tag2task WHERE " . implode(' AND ', $tagAnd) . ")";
            }

            // Exclude tags
            if (count($tagExIds) > 0) {
                $sqlWhere .= "\n AND todo.id NOT IN (SELECT DISTINCT task_id FROM {$db->prefix}tag2task " . "WHERE tag_id IN (" . implode(',', $tagExIds) . "))";
            }
        }
        ...
        if ($sort == 0) {
            ...
            $sqlSort .= "ow DESC";                                    // byHand (reverse)
        } elseif ($sort == 1) {
            $sqlSort .= "prio DESC, ddn ASC, duedate ASC, ow ASC";      // byPrio
        } elseif ($sort == 101) {
            $sqlSort .= "prio ASC, ddn DESC, duedate DESC, ow DESC";  // byPrio (reverse)
        } elseif ($sort == 2) {
            $sqlSort .= "ddn ASC, duedate ASC, prio DESC, ow ASC";      // byDueDate
        } elseif ($sort == 102) {
            $sqlSort .= "ddn DESC, duedate DESC, prio ASC, ow DESC";  // byDueDate (reverse)
        } elseif ($sort == 3) {
            $sqlSort .= "d_created ASC, prio DESC, ow ASC";             // byDateCreated
        } elseif ($sort == 103) {
            $sqlSort .= "d_created DESC, prio ASC, ow DESC";          // byDateCreated (reverse)
        } elseif ($sort == 4) {
            $sqlSort .= "d_edited ASC, prio DESC, ow ASC";              // byDateModified
        } elseif ($sort == 104) {
            $sqlSort .= "d_edited DESC, prio ASC, ow DESC";           // byDateModified (reverse)
        } elseif ($sort == 5) {
            $sqlSort .= "title ASC, prio DESC, ow ASC";                 // byTitle
        } elseif ($sort == 105) {
            $sqlSort .= "title DESC, prio ASC, ow DESC";              // byTitle (reverse)
        } else {
            $sqlSort .= "ow ASC";
        }
