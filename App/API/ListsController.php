...
class ListsController extends ApiRequestResponse
{
    ...
    public function putId($id)
    {
        ...
        switch ($action) {
            case 'rename':
                $this->response->data = $this->renameList($id);
                break;
            case 'sort':
                $this->response->data = $this->sortList($id);
                break;
            case 'publish':
                $this->response->data = $this->publishList($id);
                break;
            case 'enableFeedKey':
                $this->response->data = $this->enableFeedKey($id);
                break;
            case 'showNotes':
                $this->response->data = $this->showNotes($id);
                break;
            case 'hide':
                $this->response->data = $this->hideList($id);
                break;
            case 'clearCompleted':
                $this->response->data = $this->clearCompleted($id);
                break;
            case 'delete':
                $this->response->data = $this->deleteList($id);
                break; //compatibility
    ...
    public static function setListSortingById(int $listId, int $sort)
    {
        $db = DBConnection::instance();
    ...
    public static function setListShowCompletedById(int $listId, bool $showCompleted)
    {
        $db = DBConnection::instance();
        if ($listId == -1) {
            $opts = Config::requestDomain('alltasks.json');
