...
class MTTSessionHandler implements SessionHandlerInterface
{
    ...
    public function read($id)
    {
        ...
        if ($r[1] < $time) {
            ...
            $this->db->ex("UPDATE {$this->db->prefix}sessions SET last_access=?,expires=? WHERE id = ?", array($time, $expire, $id));
        }
        return $r[0];
    }
    ...
    public function write($id, $data): bool
    {
        ...
        if (!$exists) {
            ...
            $expire = time() + 14 * 86400;
            $this->db->ex("INSERT INTO {$this->db->prefix}sessions (id,data,expires) VALUES (?,?,?)", array($id, $data, $expire));
        } else {
            // Update existing session
            $this->db->ex("UPDATE {$this->db->prefix}sessions SET data = ? WHERE id = ?", array($data, $id));
        }
        return true;
    }
