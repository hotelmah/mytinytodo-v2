...
class MTTSmartSyntax implements MTTSmartSyntaxInterface
{
    ...
    private function findDuedate(string $s): ?string
    {
        if (preg_match("|^(\d+)([dwmy]{1})$|", $s, $m)) { // 5d,2w...
            ...
            } elseif ($period == 'm' || $period == 'y') { //months,years
                if ($period == 'y') {
                    $count *= 12;
                }
    ...
    public static function parseDuedate(string $s): ?string
    {
        ...
        if (max((int)strpos($df2, 'n'), (int)strpos($df2, 'm')) > max((int)strpos($df2, 'd'), (int)strpos($df2, 'j'))) {
            $formatDayFirst = true;
        } else {
            $formatDayFirst = false;
        }
        ...
        if (preg_match("|^(\d+)-(\d+)-(\d+)\b|", $s, $ma)) {
            ...
            $m = (int)$ma[2];
            $a = explode(',', date('Y,m,d'));
            if ($m < (int)$a[1] || ($m == (int)$a[1] && $d < (int)$a[2])) {
                $y = (int)$a[0] + 1;
            } else {
                $y = (int)$a[0];
            }
        ...
        if ($y < 100) {
            $y = 2000 + $y;
        } elseif ($y < 1000 || $y > 2099) {
            $y = 2000 + (int)substr((string)$y, -2);
        }
        if ($m > 12) {
            $m = 12;
        }
    ...
    public static function parseSmartSyntax(string $title): ?array
    {
        $a = MTTSmartSyntax::instance()->parse($title);
        MTTFilterCenter::doFilter('parseSmartSyntax', $title, $a);
        return $a;
    }
}
